...
public sealed class SecurityHeadersAttribute : ActionFilterAttribute
{
    public override void OnResultExecuting(ResultExecutingContext context)
    {
        ...
        if (result is PageResult)
        {
            ...
            // Browser refresh uses a random localhost WebSocket port while developing.
            // Keep the production policy strict while allowing that development tooling.
            var environment =
                context.HttpContext.RequestServices.GetRequiredService<IHostEnvironment>();
            if (environment.IsDevelopment())
            {
                csp += " connect-src 'self' ws://localhost:* wss://localhost:*;";
            }
